$(function() {
    ...
    $('a.package_lesson_more').on('click', function(e) {
        e.preventDefault();
        var ename = $(this).data('package-ename');
        $(this).parents('.lesson_table_'+ename).find('tr:hidden').fadeIn(500);
    ...
    if ($('ul.hash_linked_tabs')[0]) {
        ...
        $('ul.hash_linked_tabs').on('click', 'li', function(e) {
            if ($(this).attr('id')) {
    ...
    $('#change_to_smartphone_view').click(function(e) {
        e.preventDefault();
        document.cookie = 'display_pc_view=0;';
        location.reload();
    });
    ...
    // hide memo if cookie is set.
    if ($('#memo_form')[0]) {
        var hideMemo = HMHM.cookie.get('hide_memo');
        if (hideMemo && hideMemo == 1) {
            $('#memo_form').hide();
            $('#toggle_memo').text('[表示]');
        }
    }

    $('#toggle_memo').click(function() {
        if ($('#memo_form').is(':visible')) {
            HMHM.cookie.set('hide_memo', 1, 86400*90*1000, '/lessons/');
            $('#memo_form').fadeOut();
            $(this).text('[表示]');
        } else {
            HMHM.cookie.set('hide_memo', '', -86400*1000, '/lessons/');
            $('#memo_form').fadeIn();
            $(this).text('[非表示]');
        }
    });
    ...
    // toggle side news
    $('#toggle_side_news_body').click(function() {
...
HMHM.quiz = {};
HMHM.quiz.results = [];
HMHM.quiz.setResult = function(result) {
    HMHM.quiz.results.push(result);
};
HMHM.quiz.sendResults = function(callback) {
    $.ajax({
        url:'/quiz/create_history.json',
        type:'POST',
        data:{
            results:HMHM.quiz.results,
            user_id:$('#user_id').val(),
...
HMHM.analytics = {};
HMHM.analytics.trackEvent = function(category, action, label, value) {
...
HMHM.news = {};
HMHM.news.hide = function() {
    $('#side_news_body').hide();
    $('#side_news_more').hide();
    $('#side_news_header').addClass('body-hidden');
    $('#toggle_side_news_body').text('開く');
};
HMHM.news.show = function() {
    $('#side_news_body').show();
    $('#side_news_more').show();
    $('#side_news_header').removeClass('body-hidden');
    $('#toggle_side_news_body').text('閉じる');
};
